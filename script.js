...
async function cargarPorPlataforma(providerId) {
    try {
        ...
        const data = await res.json();
        pintarPeliculas(data.results.slice(0, 10), true); // true = Pinta la medalla roja
    } catch (error) {
        mostrarError();
    }
}
...
async function ejecutarBusqueda() {
    ...
    try {
        ...
        const data = await res.json();
        pintarPeliculas(data.results, false); 
    } catch (error) {
        mostrarError();
    }
}
...
async function cargarRandom() {
    ...
    try {
        ...
        const peli = data.results[randomIndex];
        
        const peliPoster = peli.poster_path ? `${IMG_URL}${peli.poster_path}` : 'https://via.placeholder.com/500x750?text=Sin+Poster';
        
        // Pintamos la tarjeta gigante
        randomShowcase.innerHTML = `
            <img src="${peliPoster}" alt="${peli.title}" class="showcase-img">
            <div class="showcase-info">
                <h3>${peli.title}</h3>
                <p>${peli.overview || "Esta película no tiene descripción en la base de datos, ¡pero seguro que es una joya oculta!"}</p>
                <button class="btn-hero-primary" style="width: fit-content;" onclick="window.open('https://www.themoviedb.org/movie/${peli.id}', '_blank')">
...
function pintarPeliculas(peliculas, mostrarMedallaTop = false) {
    ...
    peliculas.forEach((peli, index) => {
        ...
        card.innerHTML = `
            ${rankingHtml}
            <img src="${peliPoster}" alt="${peli.title}">
